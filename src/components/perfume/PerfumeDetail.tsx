...
const PerfumeDetail = ({ route, navigation }) => {
  const [favoritePerfumes, setFavoritePerfumes] = useState([]);
  const [isFavorite, setIsFavorite] = useState(false);
  const [perfume, setPerfume] = useState(null); // Initialize perfume as null
  const { perfumeId } = route.params;

  const onPressFavorite = async (item) => {
  ...
  const onPressUnfavorite = async (item) => {
  ...
  useEffect(() => {
    console.log('Subscribing to navigation focus event...');
  ...
  useEffect(() => {
    (async () => {
      try {
        const fetchedPerfume = await getPerfumeDetail(perfumeId); // Fetch perfume details
        if (fetchedPerfume) {
          setPerfume(fetchedPerfume); // Set perfume state with fetched data
          setIsFavorite(checkIsFavorite(fetchedPerfume.id));
        }
      } catch (error) {
  ...
  return (
    <ScrollView style={styles.container}>
      {perfume && ( // Render perfume details if perfume is not null
        <>
          <Image
            style={styles.image}
            resizeMode="contain"
          ...
          <View style={styles.info}>
            ...
            <View style={styles.cardStats}>
              <View>
                ...
                <View style={styles.cardStatsItem}>
                  ...
                  <Text style={styles.cardStatsItemText}>
                    {perfume.rating ? perfume.rating.toFixed(1) : '0'}/5
                  </Text>
                  <Text style={styles.cardStatsItemText}>
              ...
              <Text style={styles.cardPrice}>
                ${perfume.price ? perfume.price.toLocaleString('en-US') : '0'}
              </Text>
            </View>
